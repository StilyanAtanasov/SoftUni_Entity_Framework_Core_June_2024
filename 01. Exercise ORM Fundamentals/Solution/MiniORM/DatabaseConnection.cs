...
internal class DatabaseConnection
{
    ...
    public DatabaseConnection(string connectionString) => _connection = new SqlConnection(connectionString);

    private SqlCommand CreateCommand(string queryText, params SqlParameter[] parameters)
    {
        var command = new SqlCommand(queryText, _connection, _transaction);
        ...
        where T : class
    {
        ...
        foreach (var entity in modifiedEntities)
        {
            ...
            var updatedRows = ExecuteNonQuery(query, columnsParameters.Concat(primaryKeyParameters).ToArray());
