...
public class EfRepository : IRepository
{
    ...
    public EfRepository(BlogDbContext context) => _context = context;
    ...
    public async Task<bool> CheckExpressionAsync<T>(Expression<Func<T, bool>> expression, params Expression<Func<T, object>>[] includes) where T : class
    {
        IQueryable<T> query = DbSet<T>();
        foreach (Expression<Func<T, object>> include in includes) query = query.Include(include);
